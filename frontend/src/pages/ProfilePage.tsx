...
function ProfilePage({ profiles }: ProfilePageProps) {
	...
	// Check if current user has sent a connection request to this profile
	// Only check requests WHERE currentUserId is the sender (fromId)
	const sentRequests = connections.getSentRequests(currentUserId);
	const hasRequested = sentRequests.some(
		(req) => req.toId === profile.id && req.status === "pending"
	);
	...
	useEffect(() => {
		if (showSuccessToast) {
			...
			// Cleanup function to prevent memory leak if component unmounts
